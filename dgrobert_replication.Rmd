...
#match the axes coordinates
p <- p + coord_cartesian(ylim = c(2, 50), xlim = c(2, 50))
#add best fit line
p <- p + stat_smooth(na.rm=TRUE, method = "lm", se=TRUE, color = "black", formula = y~x)
#insert r^2 and p values
p <- p + stat_poly_eq(na.rm=TRUE, formula = log(y)~log(x), aes(label = paste(..rr.label..)), rr.digits=3, label.x="left", label.y="top")
p <- p + stat_poly_eq(na.rm=TRUE, formula = log(y)~log(x), aes(label = paste(..p.value.label..)), p.digits=3, label.x = "left", label.y="bottom")
...
p <- p + scale_color_grey() + theme_classic()
#match the axes coordinates
p <- p + coord_cartesian(ylim = c(2, 50), xlim = c(10, 500))

#insert NS label
p <- p + stat_poly_eq(formula = log(y)~log(x), aes(label = paste0("NS")), label.x="left", label.y="top", na.rm=TRUE)
