...
p <- p + scale_fill_grey()
p
#according to the fig 2 caption, the whisker extends to 1.5 times the IQR, which may be different from the ggplot default, which is "largest observation less than or equal to upper hinge+1.5*IQR" for ymax (the same for ymin).  So a single outlier shows in fig 2 that does not show up in fig 2 replicate, perhaps because the whisker length of 1.5*IQR (common to both) originates at the mean in the published version and from the hinge in ggplot2.  However, adjusting the aes(ymin) etc did not correct the problem.  Tinkered with coord_cartesian() and trans=log to no avail.  The difference is very slight except for the missing second outlier on "reduced" fuel at the soil surface (0cm).
